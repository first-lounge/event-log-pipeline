...
원본을 먼저 보존한 뒤 정규화하는 **ELT 기반 파이프라인**. 생성기가 만든 이벤트를 `raw_events`에 원본 그대로 적재(Load)한 뒤, 검증·정규화(Transform)해 분석용 테이블로 옮긴다.
...
- 이벤트 타입 4종: `page_view` / `click` / `purchase` / `error`
- Load/Transform 분리로 변환 로직 변경·버그 시 **재처리(replay)** 가능
...
- `app`(파이프라인)이 DB가 준비된 뒤 자동으로 **생성 → 적재 → 변환**을 1회 실행하고 종료됩니다(배치).
- **Grafana 대시보드**: http://localhost:3000 (익명 접근 허용, 별도 로그인 불필요)
- 생성 데이터 개수 조정: `main.py`의 `--size` 옵션 (기본값 사용)

> **스키마(init.sql) 변경 시**: PostgreSQL은 데이터 디렉토리가 비어 있을 때만 `init.sql`을 실행하므로, 스키마를 바꾸면 `docker compose down -v && docker compose up -d --build`로 볼륨을 초기화해야 반영된다. (더미 데이터라 `down -v` 무방)
...
```
raw_events ──(변환)──▶ events ──┬─ (page_view: 상세 없음)
                                ├─▶ purchase
                                ├─▶ error
                                └─▶ click
```
...
▶ (추후) A. Kubernetes manifest / B. AWS 아키텍처
