...
8. [선택 과제 A (Kubernetes)](<#8-선택-과제-A-(Kubernetes)>)
...
## 8. 선택 과제 A (Kubernetes)

### Job

#### 역할

- 이벤트 생성 -> 적재 -> 변환을 한 번 돌고 끝나는 배치입니다.

#### 선택 이유

- Deployment로 설정 시 계속해서 실행되는 무한 루프가 발생하기 때문에 한 번만 돌고 끝나는 Job을 선택하였습니다.

### StatefulSet

#### 역할

- 데이터 저장소와 저장소 이름이 고정되어 DB pod가 재생성 시에도 같은 저장소를 할당해줍니다.

#### 선택 이유

- pod가 재생성되어도 같은 저장소와 식별자를 할당받아 데이터가 유실되지 않고 유지되므로 선택하였습니다.

### Service (headless / NodePort)

#### 역할

- NodePort는 포트를 외부에 노출하여 대시보드의 외부 접근을 가능하게 해줍니다.
- headless는 StatefulSet 파드에 안정적인 DNS를 제공합니다.

#### 선택 이유

- NodePort는 포트 설정으로 간편히 외부 접속이 가능하기 때문에 테스트 목적으로 적합하다고 판단하여 선택하였습니다.
- headless에서 postgres는 내부 전용이라 외부 노출이 필요 없어 선택했습니다

### Deployment

#### 역할

- grafana pod를 관리(생성, 교체, 스케일)합니다.

#### 선택 이유

- pod를 관리(생성, 교체, 스케일)할뿐만 아니라 이미지 버전 변경 시 롤링 업데이트, 롤백을 관리할 수 있어 선택하였습니다.

### Secret

#### 역할

- DB 접속 정보 같은 민감 정보를 ConfigMap이 아닌 Secret으로 분리해 관리합니다.

#### 선택 이유

- Secret은 base64로 인코딩되고, ConfigMap보다 민감 정보에 적합합니다.

### ConfigMap

#### 역할

- init.sql과 grafana 프로비저닝 파일들과 같은 설정 파일을 pod에 주입합니다.

#### 선택 이유

- 민감 정보가 아닌 일반 설정이기 때문에 Secret 대신 ConfigMap을 선택하였습니다.
